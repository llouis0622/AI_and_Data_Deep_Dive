...
## 1. 사이킷런을 통해 학습과 예측에 사용할 데이터셋 나누기

- 라이브러리 로드
- 전처리한 데이터셋 불러오기
- 학습과 예측에 사용할 데이터셋 만들기

## 2. 랜덤값을 고정하여 디시전트리로 학습과 예측하기

- 학습과 예측하기

## 3. 최적의 max_depth 파라미터값 찾기

- 최적의 max_depth 값 찾기

## 4. GridSearchCV를 사용해서 최적의 하이퍼 파라미터 값 찾기

- cross validation : train 데이터셋을 여러 fold로 나눠서 평균을 내는 방법
- cv_results_ : cross validation 결과를 데이터프레임으로 반환

## 5. RandomSearchCV를 사용해서 최적의 하이퍼 파라미터 값 찾기

- Random Search : 좋은 성능을 낼 수 있는 랜덤값 탐색
...
- 부트스트랩 : 샘플링하여 여러 데이터를 만듦

## 2. 그라디언트 부스팅 알고리즘 사용하기

- single : 하나의 트리
- bagging : 여러 개의 트리
- boosting : 트리에 가중치 부여

## 3. RandomSearchCV로 여러 알고리즘의 최적의 하이퍼 파라미터를 찾기 1

- RandomSearchCV

## 4. RandomSearchCV로 여러 알고리즘의 최적의 하이퍼 파라미터를 찾기 2

- 모델 별로 가장 성능이 좋은 파라미터 수치 확인
- 하이퍼 파라미터 튜닝을 여러 번 할수록 좋은 성능을 얻음